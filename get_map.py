...
if __name__ == "__main__":
    ...
    #   默认指向根目录下的VOC数据集
    # -------------------------------------------------------#
    # VOCdevkit_path = './datasets/NWPU_VHR-10_dataset/positive_image_set'
    # -------------------------------------------------------#
    #   结果输出的文件夹，默认为map_out
    # -------------------------------------------------------#
    map_out_path = 'map_out'

    image_ids = open('test.txt').read().splitlines()
    ...
    if map_mode == 0 or map_mode == 1:
        print("Load model.")
        fenet = FENet(confidence=.001, nms_iou=nms_iou)
        print("Load model done.")

        print("Get predict result.")
        for image_id in tqdm(image_ids):
            img_file = image_id.split(' ')[0]
            ids = img_file.split('/')[-1].split('.')[0]
            # image_path = os.path.join(VOCdevkit_path, "VOC2007/JPEGImages/" + image_id + ".jpg")
            image = Image.open(img_file)
            if map_vis:
                image.save(os.path.join(map_out_path, "images-optional/" + ids + ".jpg"))
            fenet.get_map_txt(ids, image, class_names, map_out_path)
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for image_id in tqdm(image_ids):
            img_file = image_id.split(' ')[0]
            ids = img_file.split('/')[-1].split('.')[0]
            labels = image_id.split(' ')[1:-1]
            with open(os.path.join(map_out_path, "ground-truth/" + ids + ".txt"), "w") as new_f:
                for obj in labels:
                    obj = obj.split(',')
                    obj_name = obj[-1]
                    left = obj[0]
                    ...
                    right = obj[2]
                    bottom = obj[3]
                    new_f.write("%s %s %s %s %s\n" % (class_names[int(obj_name)], left, top, right, bottom))
